...
def get_allowed_actions(request: AirlockRequest, user: User, airlock_request_repo: AirlockRequestRepository) -> AirlockRequestWithAllowedUserActions:
    ...
    if can_review_request and user.is_in_role("AirlockManager"):
        allowed_actions.append(AirlockActions.Review)

    if can_cancel_request and (user.is_in_role("WorkspaceOwner") or user.is_in_role("WorkspaceResearcher")):
        allowed_actions.append(AirlockActions.Cancel)

    if can_submit_request and (user.is_in_role("WorkspaceOwner") or user.is_in_role("WorkspaceResearcher")):
